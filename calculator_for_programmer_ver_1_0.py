...
class ProgrammerCalculator:
    def __init__(self, root):
        self.root = root
        self.root.title("程式設計猿計算機 ver 1.0")
    ...
    def on_button_click(self, button_text):
        ...
        if button_text == '=':
            try:
                ...
                self.result_display.insert(tk.END, hex_result)  # 顯示16進制
                self.binary_display.delete(0, tk.END)
                self.binary_display.insert(tk.END, binary_result)  # 顯示2進制
            ...
            try:
                result = math.log10(int(current_text, 16))  # 將16進制轉為10進制進行計算
                hex_result = self.to_hex(int(result))  # 顯示結果為16進制，帶 '0x' 前綴
                binary_result = self.to_binary(int(result))  # 顯示對應的2進制，帶底線
                self.result_display.delete(0, tk.END)
                self.result_display.insert(tk.END, hex_result)  # 顯示16進制
            ...
            self.input_display.insert(tk.END, '**')  # Python 里 ^ 是按位異或，使用 ** 作為乘方符號
        elif button_text == '<<':
            try:
                result = int(current_text, 16) << 1  # 向左位移
                hex_result = self.to_hex(result)  # 顯示結果為16進制，帶 '0x' 前綴
                binary_result = self.to_binary(result)  # 顯示對應的2進制，帶底線
                self.result_display.delete(0, tk.END)
                self.result_display.insert(tk.END, hex_result)  # 顯示16進制
                self.binary_display.delete(0, tk.END)
            ...
            try:
                result = int(current_text, 16) >> 1  # 向右位移
                hex_result = self.to_hex(result)  # 顯示結果為16進制，帶 '0x' 前綴
                binary_result = self.to_binary(result)  # 顯示對應的2進制，帶底線
                self.result_display.delete(0, tk.END)
                self.result_display.insert(tk.END, hex_result)  # 顯示16進制
                ...
                self.result_display.insert(tk.END, "錯誤")
                self.binary_display.delete(0, tk.END)
        elif button_text == 'AND':
            self.input_display.insert(tk.END, '&')
        elif button_text == 'OR':
            self.input_display.insert(tk.END, '|')
        elif button_text == 'XOR':
            self.input_display.insert(tk.END, '^')
        elif button_text == 'NOT':
            self.input_display.insert(tk.END, '~')
        elif button_text == '0x':  # 為16進制數字加入前綴
            self.input_display.insert(tk.END, '0x')
        else:
