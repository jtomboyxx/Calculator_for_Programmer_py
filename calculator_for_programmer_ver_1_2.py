...
class ProgrammerCalculator:
    def __init__(self, root):
        self.root = root
        self.root.title("程式設計猿計算機 ver 1.2")
    ...
    def on_button_click(self, button_text):
        ...
        if button_text == '=':
            ...
            try:
                result = math.sqrt(eval(current_text))  # 將16進制轉為10進制進行計算
            ...
            try:
                # 計算時將 16進制數字轉換為10進制數字運算
                result = eval(current_text)
                result = 0xFFFFFFF ^ result

                # Convert to hex, binary and decimal
                hex_result = self.to_hex(result)
                binary_result = self.to_binary(result)
            ...
            try:
                result = eval(current_text)
                result = math.log(result, 10)  # 將16進制轉為10進制進行計算
                hex_result = self.to_hex(int(result))  # 顯示結果為16進制，帶 '0x' 前綴
                binary_result = self.to_binary(int(result))  # 顯示對應的2進制，帶底線
                decimal_result = str(result)  # 十進制顯示結果
